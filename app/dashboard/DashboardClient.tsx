    ...
    <div className="max-w-7xl mx-auto p-4 md:p-8 animate-in slide-in-from-bottom-2 duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
        <div className="flex-1">
          ...
          <p className="text-slate-500 mt-2 text-lg">Managing {initialSubmissions.length} trade verdicts.</p>
        </div>
        ...
        <div className="relative w-full md:w-80 group">
          ...
          />
          {searchTerm && (
            <button onClick={() => setSearchTerm("")} className="absolute right-3 top-1/2 -translate-y-1/2 p-1 hover:bg-slate-100 rounded-full">
      ...
      <div className="w-full space-y-4">
        {filteredSubmissions.map((sub) => {
          ...
          const isNewFeedback = sub.metadata?.contractor_test_questions && !sub.metadata?.feedback_reviewed;
          const isReviewedFeedback = sub.metadata?.contractor_test_questions && sub.metadata?.feedback_reviewed;
            ...
            >
              <div className="flex items-center gap-4 md:w-1/4">
                ...
                <div>
                  ...
                  <p className="text-sm font-bold text-slate-900">{new Date(sub.created_at).toLocaleDateString()}</p>
                </div>
              </div>

              <div className="flex-1">
                <div className="flex items-center gap-2 mb-2">
                  <Badge variant={sub.status === 'COMPLETED' ? 'default' : 'secondary'} className="text-[10px] uppercase">
                    {sub.status}
                  </Badge>
                  ...
                  {(isNewFeedback || isReviewedFeedback) && (
                    <Dialog>
                      <DialogTrigger asChild>
                        <button className={`flex items-center gap-1 px-2 py-0.5 rounded border transition-colors cursor-pointer 
                          ...
                          <span className="text-[10px] font-bold uppercase tracking-tighter">
                            {isNewFeedback ? 'Feedback Received' : 'Feedback Reviewed'}
                          </span>
                        </button>
                      </DialogTrigger>
                      <DialogContent className="sm:max-w-[525px] rounded-3xl">
                        ...
                        <div className="mt-4 space-y-4">
                          <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                            ...
                            <p className="text-sm font-bold text-slate-900">{sub.assessments?.title || 'Residential Site'}</p>
                          </div>
                          ...
                          {isNewFeedback && (
                            <Button 
                              onClick={() => handleMarkReviewed(sub.id, sub.metadata)}
      ...
      <footer className="mt-20 border-t border-slate-100 py-10 no-print">
        <div className="flex flex-col md:flex-row justify-between items-center gap-6 text-sm text-slate-500">
          ...
          <div className="flex gap-8 font-bold">
            ...
            <Link href="mailto:support@siteverdict.online" className="hover:text-blue-600">Support</Link>
